...
Write-Step "State before delete"
$usersBefore = Invoke-RestMethod -Uri "$GatewayUrl/user"
$companiesBefore = Invoke-RestMethod -Uri "$GatewayUrl/company"

($usersBefore | Where-Object { $_.id -in @($director.id, $employee.id) }) | ConvertTo-Json -Depth 5
($companiesBefore | Where-Object { $_.id -eq $company.id }) | ConvertTo-Json -Depth 5

Write-Step "Delete company"
Invoke-RestMethod -Method DELETE -Uri "$GatewayUrl/company/$($company.id)"
Write-Host "DELETE /company/$($company.id) -> 204 No Content"
...
$employeeAfter = $usersAfter | Where-Object { $_.id -eq $employee.id }
$companyAfter = $companiesAfter | Where-Object { $_.id -eq $company.id }

try {
    Invoke-RestMethod -Uri "$GatewayUrl/company/exists/$($company.id)" | Out-Null
    $existsStatus = 204
} catch {
    $existsStatus = [int]$_.Exception.Response.StatusCode
}

$employeeAfter | ConvertTo-Json -Depth 5
